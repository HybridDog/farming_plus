		...
		for divz=0,divs-1 do
			...
			for i=0,grass_amount do
				...
				for y=30,0,-1 do
					...
					--~ or not minetest.registered_nodes[nn].buildable_to then
					if nn ~= "air" then
						if nn == "default:dirt_with_grass"
						and y ~= 30 then -- the first one is not reliable
							ground_y = y
						end
						break
					end
				...
				if ground_y then
					-- test if ground_y is in a mountain
					local plant = farming_plus_registered_plants[
						math.floor(perlin1:get2d{x=x, y=z}
							* (#farming_plus_registered_plants)
						)
					]
					if plant then
						minetest.set_node({x=x,y=ground_y+1,z=z},
							{name=plant.full_grown}
						)
					end
				end
			end
